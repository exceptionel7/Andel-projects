...
async function cjFetch(endpoint, options = {}, attempt = 1) {
  ...
  try {
    ...
    if (!data.result) {
      ...
      // Retry ONLY transient throttling, with a generous delay. We intentionally
      // do NOT retry hard failures like "access disabled/frozen" — retrying those
      // just adds more requests and keeps CJ frozen. Gentle retries + caching
      // (ISR) keep our request volume low so CJ doesn't freeze our API access.
      const throttled =
        res.status === 429 ||
    ...
    // One gentle retry on network-level failures.
