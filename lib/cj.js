...
async function cjFetch(endpoint, options = {}, attempt = 1) {
  ...
  try {
    ...
    if (!data.result) {
      ...
      // CJ throttles requests aggressively. Retry transient / rate-limit errors
      // with an increasing delay so products don't silently disappear.
      const retriable =
        res.status === 429 ||
...
export async function getNavCategories() {
  ...
  try {
    ...
    const nav = firstLevels
      ...
      .filter((c) => c.name && c.slug && c.leafIds.length > 0);
